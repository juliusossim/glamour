import { MediaType, MediaSource, UseMediaResult, VideoControls, VideoState, MediaMetadata } from '@org/models';
import { useCallback, useEffect, useRef, useState } from 'react';
import { IMAGE_MIME_TYPES, VIDEO_MIME_TYPES, IMAGE_EXTENSIONS, VIDEO_EXTENSIONS } from '../utils';
...
export function useMedia(source: MediaSource): UseMediaResult {
  ...
  const requestFullscreen = useCallback(async () => {
    ...
    try {
      ...
      } else if ((video as unknown as { webkitRequestFullscreen?: () => Promise<void> }).webkitRequestFullscreen) {
        await (video as unknown as { webkitRequestFullscreen: () => Promise<void> }).webkitRequestFullscreen();
      }
    } catch (error) {
  ...
  const controls: VideoControls | null = metadata.type === 'video' ? {
    play,
    pause,
    toggle,
    seek,
